...
function SalesModel_rf({ filters }) {
  ...
  const handlePredict = async () => {
    setLoading(true);
    try {
      const res = await axios.get("http://localhost:5004/predict_sales_rf", {
      params: { 
        year: filters.year, 
  ...
  const handleRetrain = async () => {
    setLoading(true);
    try {
      await axios.post("http://localhost:5004/train_sales_rf");
  ...
  return (
    <div className="sales-container">
      <h2 className="sales-title">Monthly Sales Prediction Random Forest</h2>
